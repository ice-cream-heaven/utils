...
type PaymentOrderHistoryBack struct {
	...
	Status              int64                 `gorm:"column:status;not null;comment:状态:0 生成订单，1 支付中，2 支付未通知，3 支付已通知，-1 交易失败，-2 交易过期，-3 交易撤销" json:"status"` // 状态:0 生成订单，1 支付中，2 支付未通知，3 支付已通知，-1 交易失败，-2 交易过期，-3 交易撤销
...
func (*PaymentOrderHistoryBack) TableName() string {
	return TableNamePaymentOrderHistoryBack
}
